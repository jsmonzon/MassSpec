...
if location == "server":
    ...
    datadir = "/netb/vdbosch/jsm99/data/Mres_4_10k/"
...
def process_file(file_i):
    try:
        tree_i = jsm_visualize.Arborist(
            file=file_i,
            merger_crit=config["merger_crit"],
            fesc=config["fesc"],
...
if __name__ == "__main__":
    ...
    # Auto-generate save name from mass_cut value
    mass_cut = config["mass_cut"]
    save_name = f"/home/jsm99/data/mass_spec/MW/mass_cut_{mass_cut:.2e}"
        ...
        for sim_name, attributes in dict_dict.items():
            ...
            for attr_name, data in attributes.items():
                if np.isscalar(data) or (isinstance(data, np.ndarray) and data.shape == ()):
                    sim_group.create_dataset(attr_name, data=data)
                else:
